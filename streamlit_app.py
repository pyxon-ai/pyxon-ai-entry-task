...
def evaluate_with_cohere(cohere_client, question: str, source: str, retrieved_chunks: list) -> dict:
    ...
    retrieved_sources = [c.get('file_name', 'unknown') for c in retrieved_chunks[:3]]
    
    prompt = f"""أنت مُقيّم ذكي لجودة استرجاع المعلومات. مهمتك تقييم النتائج بشكل عادل ومنطقي.
...
## معايير التقييم:
1. **الدرجة (1-10):** 
   - 8-10: النتائج تحتوي على المعلومات المطلوبة للإجابة (حتى لو جزئياً)
   - 5-7: النتائج ذات صلة بالموضوع لكن قد لا تجيب مباشرة
   - 1-4: النتائج غير ذات صلة
   
2. **المصدر الصحيح:**
   - "نعم" إذا أي من المصادر المسترجعة تطابق أو تشبه المصدر المتوقع
   - "لا" فقط إذا كانت المصادر مختلفة تماماً
   
3. **قابلية الإجابة:**
   - "نعم" إذا يمكن الإجابة على السؤال من المحتوى المسترجع
   - "جزئياً" إذا يوجد بعض المعلومات المفيدة
   - "لا" إذا لا توجد معلومات مفيدة

## ملاحظات مهمة:
- لا تتطلب تطابق نصي حرفي، المهم الصلة الدلالية
- إذا المحتوى يتكلم عن نفس الموضوع = صلة عالية
- إذا يمكن استنتاج الإجابة = قابل للإجابة

أجب بصيغة JSON فقط:
{{"score": X, "correct_source": "نعم/لا", "answerable": "نعم/جزئياً/لا", "comment": "تعليق مختصر"}}"""
